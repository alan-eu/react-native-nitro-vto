...
object MatrixUtils {
     ...
     */
    fun distance3d(p1: Pose, p2: Pose): Float {
        val dx = p2.tx() - p1.tx()
        val dy = p2.ty() - p1.ty()
        val dz = p2.tz() - p1.tz()
    ...
    /**
     * Project a Pose to NDC coordinates using view and projection matrices.
     */
    fun projectToNdc(pose: Pose, viewMatrix: FloatArray, projMatrix: FloatArray, tempVec4: FloatArray): FloatArray {
        tempVec4[0] = pose.tx()
        tempVec4[1] = pose.ty()
        tempVec4[2] = pose.tz()
        tempVec4[3] = 1f
